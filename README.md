...
``` bash
curl -s https://raw.githubusercontent.com/hrmuwanika/odoo-14-docker-compose/master/run.sh | sudo bash -s odoo-one 10014 20014
```
...
``` bash
curl -s https://raw.githubusercontent.com/hrmuwanika/odoo-14-docker-compose/master/run.sh | sudo bash -s odoo-two 11014 21014
```
...
* Default database password (**admin_passwd**) is `abc.info`, please change it @ [etc/odoo.conf#L60](/etc/odoo.conf#L60)
